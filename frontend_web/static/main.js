...
socket.on('user_offline', data => {
    const existing = allUsersMap[data.username];
    if (existing) { existing.online = false; updateUsersList(); }
});

// Construye una fila de sala igual al desktop:
// fondo azul si soy miembro, badge rojo si hay notificaciones
function makeRoomItem(r) {
    const isMember = r.members && r.members.includes(currentUser);
    const isActive = r.id === activeRoom;
    const li = document.createElement('li');

    if (isActive)       li.classList.add('active-room');
    else if (isMember)  li.classList.add('is-member');
    ...
    if (r.notifications > 0) {
        ...
        badge.innerText   = r.notifications;
...
// Divide el sidebar en MY CHANNELS y OTHER CHANNELS igual que el desktop
function updateChannelList() {
    const myList    = document.getElementById('my-channel-list');
    const otherList = document.getElementById('other-channel-list');
    myList.innerHTML    = '';
    otherList.innerHTML = '';

    lobbyData.rooms.forEach(r => {
        const isMember = r.members && r.members.includes(currentUser);
        const li = makeRoomItem(r);
        if (isMember) myList.appendChild(li);
        else          otherList.appendChild(li);
    });
}

// Dot offline en rojo igual que el desktop (ERROR_COLOR = #E32222)
...
function simulateRoomCreation() {
    ...
    socket.emit('sim_create_room', { name, creator_username: creator.username });
}